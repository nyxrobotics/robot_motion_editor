...
from ..logic.frame_file_manager import JointData


class FrameCommander:
    def __init__(self, trajectory_commander: TrajectoryCommander):
        self.trajectory_commander = trajectory_commander
        self.initial_frame = FrameData()
        self.in_frame = FrameData()
        self.current_frame = FrameData()
        self.out_frame = FrameData()

    def set_frame(self, name: str, frame_data: FrameData):
        with self._lock:
            self._ensure_joint_names(frame_data)
